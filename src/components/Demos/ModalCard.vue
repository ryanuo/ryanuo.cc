  ...
  <VueFinalModal v-model="options.modelValue" :teleport-to="options.teleportTo"
    ...
    <h1 class="text-xl flex justify-between border-b pb-2">
      ...
      <div>
        ...
        <a v-if="options.content.readme" class="i-fluent-more-48-filled m-1" :href="options.content.readme"
          target="_blank" />
        <span class="i-material-symbols-close-small-outline cursor-pointer hover:scale-120"
    ...
    <div class="flex justify-center items-center">
      <img class="w-full md:h-160 rounded at-lg:h-70vh" :src="options.content.img"
        v-if="options.content.img && !options.content.video" />
      <iframe :src="options.content.video" v-if="options.content.video" class="w-full h-80 md:h-160 at-lg:h-70vh" />
    </div>
  </VueFinalModal>
